...
def order_tasks(tasks):
    incomplete = set(tasks) #17-19 (starts with all the tasks incomplete and no start days)
    completed = set()
    start_days = {}
    while incomplete:   #20-21 (loops over the incomplete task numbers while there are still any left)
        for task_number in incomplete:
            task = tasks[task_number]   
            if task.prerequisites.issubset(completed):  #22-23 (gets the task and checks if its prerequisites are complete)
                ...
                for prereq_number in task.prerequisites:
                    ...
                    if prereq_end_day > earliest_start_day:
                        earliest_start_day = prereq_end_day # 24-29 (computes the earliest this task can start based-
                start_days[task_number] = earliest_start_day # on the end days of its prerequisites)
                incomplete.remove(task_number)
                completed.add(task_number) # 30-32 (stores the start date and remembers that this task is done)
                break # breaks out of the loop. The loop will start if there are some incomplete tasks left
    return start_days  #returns computed dictionary 
